...
def make_bill(date: str, bill_type: str, amount: float, dedupe_id: str) -> Bill:
    return Bill(
        date=date,
        time="12:00:00",
        bill_type=bill_type,
        amount=amount,
        currency="CNY",
        category="餐饮" if bill_type == "支出" else "工资",
        payment_method="其他",
        merchant="测试商户",
        note="测试",
        original_text="测试账单",
